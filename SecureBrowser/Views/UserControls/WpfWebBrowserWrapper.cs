...
namespace PasswordBoss.Views.UserControls
{
    public class WpfWebBrowserWrapper : ContentControl
    {
        ...
        private void m_innerBrowser_LoadCompleted_(object sender, System.Windows.Navigation.NavigationEventArgs e)
        {
            ...
            try
            {
                if (m_innerBrowser != null && m_innerBrowser.Source != null)
                {
                    string src = m_innerBrowser.Source.ToString();
                    string title = ((dynamic)m_innerBrowser.Document).Title;
                    if(string.IsNullOrEmpty(title))
                    {
                        title = e.Uri.ToString().Replace("http://", "");
                        title = e.Uri.ToString().Replace("https://", "");
                    }
            ...
            //dynamic head = document.GetElementsByTagName("html")[0];
            //dynamic scriptEl = document.CreateElement("script");
            //scriptEl.text = "<!-- saved from url=(0016)http://localhost -->";
        ...
        private void BrowseGoToPageExecuted_(object sender, ExecutedRoutedEventArgs executedRoutedEventArgs)
        {
            ...
            if (urlstr != null)
            {
                if(urlstr.ToLower().Contains("http://"))
                {
                    m_innerBrowser.Navigate(urlstr);
                }
                else
                {
                    m_innerBrowser.Navigate(string.Format("http://{0}",urlstr));
                }
            }
        ...
        private static void BindableSourcePropertyChanged_(DependencyObject o, DependencyPropertyChangedEventArgs e)
        {
            ...
            if (browser != null)
            {
                ...
                //browser.NavigateToStream(resStream);
                //return;

                string uri = e.NewValue as string;
                if (!string.IsNullOrWhiteSpace(uri) && common.IsUrlValid(uri, uriKind: UriKind.RelativeOrAbsolute))
                {
                    Uri uriObj;
                    try
                    {
                        uriObj = new Uri(uri, UriKind.RelativeOrAbsolute);
                        ...
                        {
                            browser.Source = new Uri("http://" + uri);
                        }
        ...
        public static readonly DependencyProperty TitleProperty =
            DependencyProperty.Register("Title", typeof(string), typeof(WpfWebBrowserWrapper), new UIPropertyMetadata(string.Empty));
