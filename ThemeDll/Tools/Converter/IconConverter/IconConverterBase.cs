...
namespace QuickZip.Converters
{
    public abstract class IconConverterBase : IValueConverter, IMultiValueConverter 
    {
        ...
        protected static Bitmap GetFileIcon(IntPtr fileName, IconSize size)
        {
        ...
        /// </summary>
        protected static Bitmap GetFileIcon(string fileName, IconSize size)
        {
            ...
            if (!iconDic.ContainsKey(extKey))
                if (delayLoading)
                {
                    ...
                    WriteableBitmap bitmap = new WriteableBitmap(
                        iconDic.ContainsKey(fastKey) ? iconDic[fastKey] : loadBitmap(KeyToBitmap(fastKey, size)));
                    ...
                    if (size == IconSize.thumbnail &&  !String.IsNullOrEmpty(ext) && 
        ...
        private void PollThumbnailCallback(object state)
        {
            ...
            try
            {
                Bitmap origBitmap = new Bitmap(input.key);
