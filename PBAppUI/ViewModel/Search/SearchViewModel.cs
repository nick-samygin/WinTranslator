...
namespace PasswordBoss.ViewModel.Search
{
    ...
    {
        private static readonly ILogger logger = Logger.GetLogger(typeof(SearchViewModel));
        #region Properties
        ...
        public void SearchSecureItems(string search)
        {
            ...
            if(!string.IsNullOrWhiteSpace(search))
            {
                ...
                watch.Stop();
                logger.Debug("SearchViewModel.GetSecureItemsSearchResult: elapsed {0}", watch.ElapsedMilliseconds);
            }
