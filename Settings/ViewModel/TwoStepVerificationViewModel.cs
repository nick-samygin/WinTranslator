...
namespace Settings.ViewModel.ViewModel
{
    public class TwoStepVerificationViewModel: ViewModelBase
    {
        ...
        public void CloseTwoStepVerificationDialog(object obj)
        {
            ...
            if(AccountSettingsViewModel.TwoStepVerification)
            {
                ...
                var resp = api.UpdateAccount(pbData.ActiveUser + "|" + pbData.DeviceUUID, new PasswordBoss.WEBApiJSON.AccountRequest { multi_factor_authentication = AccountSettingsViewModel.TwoStepVerification });
        ...
        public dynamic LoadSecretKey()
        {
            ...
            lock (SecretKeyLoadedLock)
            {
                ...
                //{
                    var pbData = this.AccountSettingsViewModel.resolver.GetInstanceOf<IPBData>();
                    IPBWebAPI api = this.AccountSettingsViewModel.resolver.GetInstanceOf<IPBWebAPI>();
                    twoStepResp = api.GetAccount2StepInfo(pbData.ActiveUser + "|" + pbData.DeviceUUID, new PasswordBoss.WEBApiJSON.AccountRequest { storage_region = this.AccountSettingsViewModel.CurrentStorageRegionUUID, email = pbData.ActiveUser, installation = pbData.InstallationUUID, synchronize = this.AccountSettingsViewModel.EnableStorageCloudBackup });
                //}
            }
        ...
        public bool ConfirmCode()
        {
            ...
            var pbData = this.AccountSettingsViewModel.resolver.GetInstanceOf<IPBData>();
            IPBWebAPI api = this.AccountSettingsViewModel.resolver.GetInstanceOf<IPBWebAPI>();
            var twoStepResp = api.ConfirmAccount2StepSecretKeyCode(pbData.ActiveUser + "|" + pbData.DeviceUUID, new PasswordBoss.WEBApiJSON.SecretKeyConfirmationRequest { code=ConfirmationCode });
        ...
        public void ShowTwoStepBackupSecurityCodeScreen(object obj)
        {
            ...
            if(!string.IsNullOrWhiteSpace(MobilePhone))
            {
                IPBWebAPI api = this.AccountSettingsViewModel.resolver.GetInstanceOf<IPBWebAPI>();
                var pbData = this.AccountSettingsViewModel.resolver.GetInstanceOf<IPBData>();
                var resp = api.UpdateAccount(pbData.ActiveUser + "|" + pbData.DeviceUUID, new PasswordBoss.WEBApiJSON.AccountRequest { backup_phone = MobilePhone });
            }
        }
